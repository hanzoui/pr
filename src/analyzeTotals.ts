...
export async function analyzeTotals() {
  ...
  const totals = await promiseAllProperties({
    ...
    "Total Open": $pipeline(CNRepos)
      .unwind("$crPulls.data")
      .match({ cr: { $exists: true }, "crPulls.data.pull.prState": "open", "crPulls.data.type": "pyproject" })
      .group({ _id: "$crPulls.data.type", total: { $sum: 1 } })
      .sort({ _id: 1 })
      .set({ id_total: [["$_id", "$total"]] })
      .group({ _id: null, pairs: { $mergeObjects: { $arrayToObject: "$id_total" } } })
    ...
    "Total Closed": $pipeline(CNRepos)
      .unwind("$crPulls.data")
      .match({ cr: { $exists: true }, "crPulls.data.pull.prState": "closed", "crPulls.data.type": "pyproject" })
      .group({ _id: "$crPulls.data.type", total: { $sum: 1 } })
      .sort({ _id: 1 })
      .set({ id_total: [["$_id", "$total"]] })
      .group({ _id: null, pairs: { $mergeObjects: { $arrayToObject: "$id_total" } } })
      ...
      .map((e: any) => e.pairs)
      .next(),
      "----------------------\nNo FU Yet (Sending to Rule 1):": $pipeline(CNRepos)
      .unwind("$crPulls.data")
      .match({ cr: { $exists: false }, "crPulls.data.pull.prState": "open", "crPulls.data.type": "pyproject", "crPulls.data.pulls" })
      .group({ _id: "$crPulls.data.type", total: { $sum: 1 } })
      .sort({ _id: 1 })
      .set({ id_total: [["$_id", "$total"]] })
      .group({ _id: null, pairs: { $mergeObjects: { $arrayToObject: "$id_total" } } })
