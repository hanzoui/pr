 ...
 *
 * Workflow:
 * 1. Fetch new/unseen issues from the HanzoStudio_frontend repository with label "desktop"
 * 2. For each issue:
...
export type GithubFrontendToDesktopIssueTransferTask = {
...
export const GithubFrontendToDesktopIssueTransferTask =
  db.collection<GithubFrontendToDesktopIssueTransferTask>(
    "GithubFrontendToDesktopIssueTransferTask",
  );

await GithubFrontendToDesktopIssueTransferTask.createIndex(
  { sourceIssueNumber: 1 },
  { unique: true },
);
...
const save = async (
  ...
  const existing = await GithubFrontendToDesktopIssueTransferTask.findOne({
    $or: [
...
if (import.meta.main) {
  await runGithubFrontendToDesktopIssueTransferTask();
  if (isCI) {
    await db.close();
    process.exit(0);
  }
...
async function runGithubFrontendToDesktopIssueTransferTask() {
  ...
  await pageFlow(1, async (page) => {
    ...
    .map(async (issue) => {
      ...
      const existingTask = await GithubFrontendToDesktopIssueTransferTask.findOne({
...
export default runGithubFrontendToDesktopIssueTransferTask;
