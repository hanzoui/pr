...
const daysDifference = Math.floor((new Date(newData.date).getTime() - new Date(oldData.date).getTime()) / (1000 * 60 * 60 * 24));
...
console.log(`  Cloneable repos: ${newData.cloneableRepoCount}/${newData.allRepoCount} (${insights.cloneableRepoPercentage})`);
console.log(`  Non-cloneable: ${insights.nonCloneableRepos} repos\n`);

console.log('👤 USERNAME GROWTH:');
console.log(`  Total increase: ${differences.usernameCount.absolute} usernames (${differences.usernameCount.percentage})`);
