...
if (import.meta.main) {
  ...
  if (values.url) {
    ...
    if (!values.channel || !values.ts) {
      ...
      console.error("   or: bun bot/slack/msg-read-nearby.ts --url <slack_url> [--before <number>] [--after <number>]");
      console.error(
...
export async function readNearbyMessages(channel: string, ts: string, before: number = 10, after: number = 10) {
  try {
