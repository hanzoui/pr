...
if (import.meta.main) {
  ...
  if (!values.channel || !values.ts) {
    console.error("Usage: bun bot/slack/msg-read-thread.ts --channel <channel_id> --ts <thread_ts> [--limit <number>]");
    console.error("Example: bun bot/slack/msg-read-thread.ts --channel C123ABC --ts 1234567890.123456 --limit 50");
    process.exit(1);
  }
