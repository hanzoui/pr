  ...
  try {
    ...
    if (truncatedParams.blocks) {
      truncatedParams.blocks = truncatedParams.blocks.map((block) => {
        if (block.type === "markdown" && block.text && block.text.length > SLACK_MARKDOWN_BLOCK_LIMIT) {
          return {
            ...block,
    ...
    return await slack.chat.postMessage(truncatedParams);
  } catch (error: any) {
    ...
    if (error?.data?.error === "msg_too_long") {
      ...
      return await slack.chat.postMessage(aggressiveParams);
    }
...
export async function safeSlackUpdateMessage(
  ...
  try {
    ...
    return await slack.chat.update(truncatedParams);
  } catch (error: any) {
    // If we still get msg_too_long error, retry with more aggressive truncation
    if (error?.data?.error === "msg_too_long") {
      console.warn("Slack msg_too_long error, retrying with aggressive truncation");
      ...
      return await slack.chat.update(aggressiveParams);
    }
