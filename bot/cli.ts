...
async function main() {
  const argv = yargs(hideBin(process.argv))
    ...
    .command(
      ...
      async (args) => {
        await loadEnvLocal();
        const { results, total, hasMore } = await searchNotion(args.query as string, (args.limit as number) ?? 10);
        console.log(`Found ${results.length} of ${total}${hasMore ? "+" : ""} results for: "${args.query}"\n`);
